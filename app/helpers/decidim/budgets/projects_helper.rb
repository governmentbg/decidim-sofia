  ...
  module Budgets
    ...
    module ProjectsHelper
      ...
      def current_rule_call_for_action_text
        ...
        return "" if current_order.projects.try(:first).try(:budget).try(:vote_selected_projects_maximum).to_i > 0

        text = ""
        if current_order_minimum_reached?
          text = t("minimum_reached", scope: "decidim.budgets.projects.order_progress.dynamic_help")
        elsif current_order.projects.empty?
          text = t("start_adding_projects", scope: "decidim.budgets.projects.order_progress.dynamic_help")
        else
          text = t("keep_adding_projects", scope: "decidim.budgets.projects.order_progress.dynamic_help")
        end
