...
module Decidim
  module Budgets
    ...
    class BudgetsListCell < BaseCell
      AVAILABLE_ORDERS = %w( highest_cost lowest_cost).freeze
      ...
      def budget_paginate
        #changed
        @budget_paginate ||= Kaminari.paginate_array(budgets).page(params[:page]).per( params[:per_page].present? ? params[:per_page] : Decidim::Paginable::OPTIONS.first )
      end
      ...
      def reorder(budgets)
        case order
        when "highest_cost"
          budgets.reorder(total_budget: :desc)
        when "lowest_cost"
          budgets.reorder(total_budget: :asc)
        when "random"
